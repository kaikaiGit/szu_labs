...
void mainWindow_key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// @TODO: Task4 添加更多交互 1~9增减反射系数（1~3已写好），0增减高光指数
		...
		glm::vec4 diffuse = mesh->getDiffuse(); // 获取物体的漫反射系数
		tmp = diffuse.x; // 获取当前的漫反射系数的 r 分量
		diffuse.x = std::min(tmp + 0.1, 1.0); // 增加漫反射系数
		mesh->setDiffuse(diffuse); // 设置物体的漫反射系数
	}
...
int main(int argc, char** argv)
{
	...
	GLFWwindow* mainwindow = glfwCreateWindow(700, 700, "2022150168_吴嘉楷_实验3.4", NULL, NULL);
	if (mainwindow == NULL)
	{
