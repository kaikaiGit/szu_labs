...
void upper_arm(glm::mat4 modelView)
{
	...
	glm::mat4 instance = glm::mat4(1.0);
	instance = glm::translate(instance, glm::vec3(0.0, UPPER_ARM_HEIGHT / 2, 0.0)); // 平移至合适位置
	instance = glm::scale(instance, glm::vec3(UPPER_ARM_WIDTH, UPPER_ARM_HEIGHT, UPPER_ARM_WIDTH)); // 按长宽高缩放正方体，形成长方体1
...
void lower_arm(glm::mat4 modelView)
{
	...
	glm::mat4 instance = glm::mat4(1.0);
	instance = glm::translate(instance, glm::vec3(0.0, LOWER_ARM_HEIGHT / 2, 0.0)); // 平移至合适位置
	instance = glm::scale(instance, glm::vec3(LOWER_ARM_WIDTH, LOWER_ARM_HEIGHT, LOWER_ARM_WIDTH)); // 按长宽高缩放正方体，形成长方体2
...
// #endif
	...
	// 大臂变换矩阵
	modelView = glm::translate(modelView, glm::vec3(0.0, BASE_HEIGHT, 0.0)); // 稍微上移一下，让大臂与底座相连
	modelView = glm::rotate(modelView, glm::radians(Theta[UpperArm]), glm::vec3(0.0, 0.0, 1.0));// 大臂旋转矩阵

	// 绘制大臂
	upper_arm(modelView); // 绘制大臂

	// 小臂变换矩阵
	modelView = glm::translate(modelView, glm::vec3(0.0, UPPER_ARM_HEIGHT, 0.0)); // 稍微上移一下，让小臂与大臂相连
	modelView = glm::rotate(modelView, glm::radians(Theta[LowerArm]), glm::vec3(0.0, 0.0, 1.0));// 小臂旋转矩阵

	// 绘制小臂	
	lower_arm(modelView); // 绘制小臂
...
int main(int argc, char **argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(700, 700, "2022150168_吴嘉楷_实验补充1", NULL, NULL);
	if (window == NULL)
	{
