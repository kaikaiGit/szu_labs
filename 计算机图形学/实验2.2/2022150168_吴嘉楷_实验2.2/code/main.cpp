...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	else if (key == GLFW_KEY_4 && action == GLFW_PRESS && mode == 0x0000)
	{
		...
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); // 线绘制模式
	}
	// @TODO: Task5:关闭线绘制模式
	else if (key == GLFW_KEY_4 && action == GLFW_PRESS && mode == GLFW_MOD_SHIFT)
	{
		cout << "line mode: disable" << endl;
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); // 填充绘制模式
	}
}
...
int main(int argc, char** argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(800, 800, "2022150168_吴嘉楷_实验2.2", NULL, NULL);
	if (window == NULL)
	{
