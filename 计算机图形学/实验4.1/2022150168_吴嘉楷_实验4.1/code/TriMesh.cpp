...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	float step = 2 * M_PI / num_samples; // 每个切片的弧度
	// 生成下表面的顶点坐标，法向量和颜色
	float z = 0.0; // 表示圆盘在z = 0的平面上
	for (int i = 0; i < num_samples; i++) {
		float theta = i * step; // 弧度
		float x = radius * cos(theta); // x坐标
		float y = radius * sin(theta); // y坐标
	...
	for (int i = 0; i < num_samples; i++) {
		...
		// 将 0到360° 映射到 UV坐标的0到1
		for (int j = 0; j < 2; j++) {
			float theta = (i + j) * step;
			float x = cos(theta) / 2.0 + 0.5; // 除以2是为了确保将半径映射到0到1
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	for (int i = 0; i < num_samples; i++) {
		...
		vertex_colors.push_back(normalize(glm::vec3(x, y, z)));	// 添加颜色，这里颜色和法向量一样
	}
	// 生成圆锥顶部的顶点坐标，法向量和颜色
